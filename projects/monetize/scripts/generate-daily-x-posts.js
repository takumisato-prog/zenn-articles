...
async function generatePosts(apiKey, articles, newsHeadlines) {
  ...
  // 曜日を取得（静岡ネタの判定用）
  const dayOfWeek = new Date().getDay(); // 0=日, 1=月, 2=火, 3=水, 4=木, 5=金, 6=土
  const isShizuokaDay = dayOfWeek === 4; // 木曜日は静岡ネタ固定
  const isDXDay = dayOfWeek === 0; // 日曜日はDX支援ネタ

  const prompt = `あなたはAIスタートアップ「Altus」のX（Twitter）担当です。
アカウント名: @altus_ai_jp「Altus | 静岡発のAI会社」
コンセプト: 静岡発・Claude Codeで47名のAIエージェント会社を作り、本当に稼げるか実験中。全記録を公開。静岡の中小企業のDX支援も行う。
...
- 投稿4（18時用）: ${isShizuokaDay ? '【静岡ネタ必須】静岡の中小企業・製造業・農業などでAIを活用した例や提案（#静岡 #静岡DX を必ず入れる）' : isDXDay ? '【DX支援ネタ】静岡の企業へのDX支援・AI活用の提案（#静岡ビジネス を入れる）' : '上記の最新AIニュースから1つ選んでコメント付きで紹介（自分の視点必須）'}
- 投稿5（21時用）: 記事告知 or 本音・今日の振り返り

ルール:
- 各投稿は140文字以内（日本語。短いほど読まれる）
- 改行を効果的に使う（3〜5行が理想）
- 全国向けハッシュタグ: #ClaudeCode #AI副業 #AIエージェント から最大2個
- 静岡向けハッシュタグ: #静岡 #静岡DX #静岡ビジネス #しずおか（投稿4が静岡ネタの日は必須）
- 「等身大のリアルな発信」が基本。過度な宣伝・大げさな表現NG
- 投稿3は必ず「？」で終わる問いかけにする
- 投稿4でニュース紹介の場合はURLを含めず、内容を要約して自分のコメントを添える
- 投稿番号と本文のみ出力（説明不要）
