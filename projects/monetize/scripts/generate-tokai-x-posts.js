...
■ 投稿タイプ指定:
- 投稿1（9:00用）: 本日テーマのメイン情報（具体的な地名・スポット名・料理名を含む実践的な情報）
- 投稿2（13:00用）: 地元民あるある・豆知識・「知らなかった！」となる視点
- 投稿3（16:00用）: 問いかけ型（必ず「？」で終わる。フォロワーがリプライしたくなる質問）
- 投稿4（20:00用）: おすすめランキング・比較・まとめ型（「〇〇といえばこれ！」系）
- 投稿5（22:00用）: 明日・週末の予告や期待感を高める投稿（次も見たくなる締め）
...
- ハッシュタグは1投稿最大5個まで
- 投稿3は必ず「？」で終わること
- 実在しない店名・スポット名は使わない（一般的な表現か実在する有名スポットのみ）
- ビジネス・仕事・AI・DXの話題は禁止（消費者向け生活情報のみ）
- 政治・社会問題・宗教への言及禁止
